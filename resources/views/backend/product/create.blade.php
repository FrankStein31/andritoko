    ...
    <div class="card-body">
      <form method="post" action="{{route('product.store')}}">
        ...
        <div class="form-group">
            ...
            <select name="satuan_id" id="satuan_id" class="form-control">
                ...
                @foreach($satuans as $satuan)
                    <option value='{{$satuan->satuan_id}}' data-weight="{{$satuan->weight}}">{{$satuan->nama_satuan}}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input id="weight" type="number" name="weight" step="0.01" readonly class="form-control">
        </div>
...
$(document).ready(function() {
      $('#satuan_id').change(function() {
          ...
          $.ajax({
              url: "/admin/get-jumlah-satuan/" + selectedOption.val(),
              method: 'GET',
              success: function(response) {
                  var options = '<option value="">--Pilih Jumlah Satuan--</option>';
                  $.each(response, function(key, value) {
                      options += '<option value="' + value + '" data-weight="' + (weight * value) + '">' + value + '</option>';
                  });
      ...
      // Set initial weight if a satuan is selected on page load
      var initialSatuanOption = $('#satuan_id').find('option:selected');
      if (initialSatuanOption.length) {
          $('#weight').val(initialSatuanOption.data('weight'));
      }
  });
</script>
